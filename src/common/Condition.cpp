...
#else
	int ret = pthread_cond_wait(&m_cond, mutex->InnerMutex());
	if (ret != 0) { 
	std::cout << "pthread_cond_wait return " << ret << std::endl; 
	if (ret == ETIMEDOUT) { std::cout << "pthread_cond_wait is timeout " << std::endl; }
	}
	return ret == 0;
#endif
}
